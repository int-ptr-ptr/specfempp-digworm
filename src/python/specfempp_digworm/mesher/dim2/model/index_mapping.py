...
class IndexMapping(IndexMappingBase):
    """An enumeration of a set of (nonconsecutive) integer tags.
    ...
    """Equal to `numpy.argsort(original_tag_list)`.

    This stores the indices of
    original_tag_list in order of increasing values.
    """

    _sorted_tag_list: np.ndarray
    """Equal to `original_tag_list[sorted_tag_inds]`.

    This stores the sorted list of tags, which
    ...
    def __init__(self, node_tags: np.ndarray):
        ...
        object.__setattr__(
            self, "_sorted_tag_list", node_tags[self._sorted_tag_inds]
        )

    @override
    def apply(self, arr) -> np.ndarray:
        """Remaps tags from the original set to the enumerated set.
        ...
        """
        return self._sorted_tag_inds[
            np.searchsorted(self._sorted_tag_list, arr)
        ]
...
class JoinedIndexMapping(IndexMapping):
    """An index mapping that combines two sets of indices.
    ...
    def __init__(self, left: IndexMapping, right: IndexMapping):
        super().__init__(
            np.unique_values(
                np.concatenate(
                    [left.original_tag_list, right.original_tag_list]
                )
            )
        )
        ...
        object.__setattr__(
            self,
            "left_to_joined",
            ComposedIndexMapping(self, left.inverse_mapping()),
        )
        object.__setattr__(
            self,
            "right_to_joined",
            ComposedIndexMapping(self, right.inverse_mapping()),
        )
