...
@dataclass
class Model:
    """Result generated from LayeredBuilder.

    This does not need gmsh to be initialized.
    """
    ...
    """
    Stores the original tags for each node.

    This is used when merging models, where
...
[physical groups](https://gmsh.info/doc/texinfo/gmsh.html#Elementary-entities-vs-physical-groups)
    ...
    """
    A 2x3 array specifying the bounds of the model.

    bbox[0,:] are the minimums on each axis,
    ...
    def union(model1: "Model", model2: "Model") -> "Model":
        """Joins two model instances.

        If two surfaces are to be exported into the same mesh,
        then call this.
        """
        ...
        bdspec1 = model1.boundaries.remapped_elements(
            elem_ind_remap.left_to_joined
        )
        bdspec2 = model2.boundaries.remapped_elements(
            elem_ind_remap.right_to_joined
        )
        ...
        for name in (
            model1.physical_groups.keys() | model2.physical_groups.keys()
        ):
        ...
        for a_entity in bdspec1.boundary_entity_spec:
            for b_entity in bdspec2.boundary_entity_spec:
                ncis.concatenate(
    ...
    def from_meshed_surface(
        ...
        """Recovers a Model from a gmsh surface.
        ...
        if isinstance(surface, int):
            ...
            gmsh.for_element_types_in_entity(
                ...
                surface,
                {
                    ...
                    16: on_MSH_QUA_8,
                    10: on_MSH_QUA_9,
                    2: lambda _a, _b: on_mesh_tri("3-node triangle"),
                    9: lambda _a, _b: on_mesh_tri("6-node 2nd order triangle"),
                    20: lambda _a, _b: on_mesh_tri(
                        "9-node 3rd order incomplete triangle"
                    ),
                    21: lambda _a, _b: on_mesh_tri(
                        "10-node 3rd order triangle"
                    ),
                    22: lambda _a, _b: on_mesh_tri(
                        "12-node 4th order incomplete triangle"
                    ),
                    23: lambda _a, _b: on_mesh_tri(
                        "15-node 4th order triangle"
                    ),
                    24: lambda _a, _b: on_mesh_tri(
                        "15-node 5th order incomplete triangle"
                    ),
                    25: lambda _a, _b: on_mesh_tri(
                        "21-node 5th order triangle"
                    ),
                },
            )
            ...
            for name in (
                []
                if physical_group_captures is None
                else physical_group_captures
            ):
        ...
        return Model.union(
            Model.from_meshed_surface(
                surface[0],
                gmsh,
                physical_group_captures=physical_group_captures,
            ),
            Model.from_meshed_surface(
                surface[1:],
                gmsh,
