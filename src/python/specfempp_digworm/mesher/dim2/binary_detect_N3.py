...
def _union_intersections(intersectlo, intersecthi, delta):
    """Interleaves two lists of intersections by endpoints.
...
def _concat_intersections(intersectlo, intersecthi, delta):
    """Concatenates two lists of intersections by endpoints.

    takes the union of two intersections, but all points of
...
def _find_intersection(  # noqa: PLR0911, PLR0913
    ...
    """Helper function for finding intersections between two edges.

    Computes the points [s1,s2,...,sn], where param1lo <= s1 < ... < sn <= param1hi,
    ...
    ysepmin = (
        0
        if (ysep12 <= 0 and ysep21 <= 0)
        else (ysep21 if ysep12 <= 0 else ysep12)
    )
    ...
    length = sum(
        b - a
        for a, b in zip(intersections[::2], intersections[1::2], strict=True)
    )
    return True if length > return_threshold else intersections


def quadratic_beziers_intersect(  # noqa: PLR0912, PLR0913
    edge1: np.ndarray,
    edge2: np.ndarray,
    ...
    """Checks the intersection between two quadratic edges.
    ...
    for isubdiv1 in range(3):
        ...
        for isubdiv2 in range(3):
            ...
            length = sum(
                b - a
                for a, b in zip(
                    tmp_intersects[::2], tmp_intersects[1::2], strict=True
                )
            )
            if length > return_threshold:
                return True
        intersects = _concat_intersections(
            intersects, tmp_intersects, return_threshold
        )
        length = sum(
            b - a
            for a, b in zip(intersects[::2], intersects[1::2], strict=True)
        )
        if length > return_threshold:
            return True
