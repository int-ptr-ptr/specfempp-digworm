...
try:
    import gmsh as _gmsh_module  # type: ignore  # noqa: PGH003
...
class GmshContext:
    ...
    def __enter__(self):
        global _ACTIVE_ENVIRONMENT  # noqa: PLW0603
        if gmsh_inactive:
    ...
    def __exit__(
        self, exc_type: type | None, exc_val: Any, exc_tb: TracebackType
    ):
        global _ACTIVE_ENVIRONMENT  # noqa: PLW0603
        # context manager:
    ...
    def __getattribute__(self, name):
        # if active, have getattr() first, then fallback to the gmsh module
        try:
            ...
            if self.is_active():
                ...
                if name in ("initialize", "finalize"):
