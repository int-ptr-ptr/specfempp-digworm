...
class PhysicalGroupBase(ABC):
    """Base class for marked ("physical group") elements.
    ...
    def remap_indices(
        ...
        """Remaps the node and element indices of this physical group.
    ...
    def remap_nodes(self, node_mapping: IndexMapping) -> "PhysicalGroupBase":
        """Apply a remap to only the node indices.
    ...
    def remap_elements(
        self, element_mapping: IndexMapping
    ) -> "PhysicalGroupBase":
        """Apply a remap to only the element indices.
    ...
    def get_all_edges(
        ...
        """Gets all edges for each element in this physical group.
...
class SingleDimensionPhysicalGroup(PhysicalGroupBase):
    """A group of elements that are only of a certain dimension.
    ...
    def __init__(  # noqa: PLR0913
        # this constructor should not be called externally
        self,
        gmsh: GmshContext,
        ...
        """Initialize to a gmsh physical group.
        ...
        if not hasattr(self.__class__, "_dim"):
            ...
            raise e
        dim = self.__class__._dim  # pyright: ignore  # noqa: PGH003
        ...
        if name is None:
            ...
            for _dim, _tag in gmsh.model.get_physical_groups():
                if (
                    _dim == dim
                    and gmsh.model.get_physical_name(_dim, _tag) == name
                ):
        ...
        for tag_ in tags:
            entities.extend(
                gmsh.model.get_entities_for_physical_group(dim, tag_)
            )

        self._populate(
            gmsh, element_nodes, node_mapping, node_coords, dim, entities
        )
    ...
    def _populate(
        ...
        for entity in entity_tags:
            gmsh.for_element_types_in_entity(
                ...
                entity,
                mapping={
                    15: lambda _elem_tags, node_tags: nodelists.append(
...
class PhysicalGroup1D(SingleDimensionPhysicalGroup):
    ...
    # consider moving this somewhere else. The information is redundant,
    # and is only stored to keep Model and PhysicalGroup in a one-way
    # relationship.
    nodes: np.ndarray
...
class UnionPhysicalGroup(PhysicalGroupBase):
    ...
    def get_all_edges(
        ...
        if collected_elems:
            return unique_edges(
                np.concatenate(collected_elems),
                np.concatenate(collected_edgetypes),
            )
        return np.empty((0,), dtype=np.uint64), np.empty((0,), dtype=np.uint8)
...
def physical_group_from_name(
    ...
    """Gets a physical group from gmsh.
    ...
    pg0d = PhysicalGroup0D(
        gmsh, element_nodes, node_mapping, node_coords, name=name
    )
    pg1d = PhysicalGroup1D(
        gmsh, element_nodes, node_mapping, node_coords, name=name
    )
