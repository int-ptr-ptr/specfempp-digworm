...
@dataclass
class BoundarySpec:
    """Tracks element edges on the boundary.
    ...
    def remapped_elements(
        self, element_index_mapping: IndexMapping
    ) -> "BoundarySpec":
    ...
    def from_model_entity(
        ...
        for model_edge_tag in edge_entity:
            ...
            def on_MSH_LIN_3(
                edge_elems, edge_nodes, model_edge_tag=model_edge_tag
            ):
                nonlocal num_collected_edges
                max_vec_size = (
                    64  # we will only vectorize up to k mesh edges at a time
                )
                ...
                for istart in range(0, n_edges, max_vec_size):
                    ...
                    unique_edges, unique_success_inds, unique_counts = (
                        np.unique(
                            successful_edges,
                ...
                if np.any(edge_counts > 2):  # noqa: PLR2004
                    rte = RuntimeError(
                    ...
                    (failtags,) = np.nonzero(edge_counts > 2)  # noqa: PLR2004
                    rte.add_note(
        ...
        edge_tags, element_inds, element_edges, rtree, entity_spec = (
            _collect_from_types(num_collected_edges, collected_values)
        )
    ...
    @staticmethod
    def union(
        bdspec1: "BoundarySpec", bdspec2: "BoundarySpec"
    ) -> "BoundarySpec":
        ...
        for entity_ind in set(bdspec1.boundary_entity_spec.keys()) | set(
            ...
            if entity_ind in bdspec1.boundary_entity_spec:
                ...
                if entity_ind in bdspec2.boundary_entity_spec:
                    ...
                    edge1_to_combined[spec1.start_index : spec1.end_index] = (
                        np.arange(edges_start, edges_mid)
                    )
                    edge2_to_combined[spec2.start_index : spec2.end_index] = (
                        np.arange(edges_mid, edges_end)
                    )
                else:
                    ...
                    edge1_to_combined[spec1.start_index : spec1.end_index] = (
                        np.arange(edges_start, edges_end)
                    )
            else:
        ...
        rtree = RTree()
        for hit in bdspec1.rtree.intersection(
            bdspec1.rtree.bounds, objects=True
        ):
            rtree.insert(int(edge1_to_combined[hit.id]), hit.bbox)
        for hit in bdspec2.rtree.intersection(
            bdspec2.rtree.bounds, objects=True
        ):
...
def _collect_from_types(num_collected_edges, collected_values):
    ...
    return edge_tags, element_inds, element_edges, rtree, entity_spec
