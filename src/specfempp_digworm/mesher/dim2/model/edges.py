...
class ConformingInterfaces:
    ...
    def join(
        ...
        if np.any(elements_new_adj == _disagreement_tag):
            rte = RuntimeError(
    ...
    def from_element_node_matrix(
        ...
        edges = edges_of_all_elements(element_node_matrix, True).reshape(
            (-1, 3)
        )
        ...
        if np.any(counts > 2):  # noqa: PLR2004
            rte = RuntimeError(
...
def edges_of_all_elements(
    ...
    """Arranges elemental node list to an edge node list.
    ...
    if consistent_order:
        return np.where(
            edges[:, :, 0, None] > edges[:, :, -1, None],
            np.flip(edges, axis=2),
            edges,
        )
    return edges
...
def vectorized_bbox_calc(node_coord_matrix: np.ndarray) -> np.ndarray:
    ...
    """
    ret = np.empty(
        node_coord_matrix.shape[:-2] + (4,), dtype=node_coord_matrix.dtype
    )
    ...
    np.maximum(
        node_coord_matrix[..., 0, :], node_coord_matrix[..., 2, :], ret[..., 2:]
    )
...
def unique_edges(
    ...
    uniques = np.unique(
        np.stack(
            [elements_arr, edge_type_arr], axis=-1, dtype=elements_arr.dtype
        ),
        axis=0,
    )
